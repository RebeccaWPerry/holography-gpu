...
# Precompile the CUDA kernel.  I dont see any good way
#  of specifying the CUDA filename so remove all cubin
#  files before compiling, and then move the compiled
#  .cubin to precompiled.cubin

from holopy.scattering.theory import gpuMie
import os
import glob
for filename in glob.glob("*.cubin"):
  os.remove(filename)

gpuMie.compile()
'''for filename in glob.glob("*.cubin"):
  binfile=filename
os.rename(binfile, "precompiled.cubin")'''
