...
kernel_holo_source = \
"""
#include </usr/lib/python2.7/dist-packages/pycuda/cuda/pycuda-complex.hpp>
#define ABS(a)  (((a) < 0) ? -(a) : (a))
typedef pycuda::complex<double> dcmplx;
__global__ void kernel_holo(const unsigned int N, const dcmplx* asbs,
  const unsigned int nstop, const unsigned int colDim, const unsigned int rowDim, const double pixelsize, 
  ...
  const double* einc, double* holo)
{
  ...
  //holoComplex = pow(abs(esx), 2.0); //+ pow(abs(esy), 2.0) + pow(abs(esz), 2.0);
